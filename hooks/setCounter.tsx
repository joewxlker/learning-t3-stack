...
export const useIncrementData = () => {
    ...
    return [count, setCounter, (greater: number, target: string, operator: boolean): React.Dispatch<React.SetStateAction<boolean>> => {
        ...
        if (!operator) {
            ...
            if (count[target] > greater)
                return setCounter(target, (count[target] - 1))
            else return setCounter(target, greater);
        };
    }] as const
};